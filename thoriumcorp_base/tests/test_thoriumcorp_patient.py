...
MOCK_PATH = 'odoo.addons.thoriumcorp_base.models.thoriumcorp.patient.date'
...
class TestPatient(TransactionCase):
    ...
    def setUp(self):
        ...
        self.patient_1 = self.env.ref('thoriumcorp_base.thoriumcorp.patient_patient_1')
        self.partner_patient_1 = self.env.\
            ref('thoriumcorp_base.res_partner_patient_1')
        self.patient_3 = self.env.ref('thoriumcorp_base.thoriumcorp.patient_patient_3')
    ...
    def test_partner_is_patient(self):
        """ Test that patient type is correctly set on partner """
        self.assertEqual(
    ...
    def test_create_vals_get_default_image_encoded(self):
        ...
        Patient = self.env['thoriumcorp.patient'].with_context(
    ...
    def test_get_default_image_encoded(self):
        ...
        Patient = self.env['thoriumcorp.patient']
    ...
    def test_check_birthdate_date(self):
        ...
        now = datetime.now()
        with self.assertRaises(ValidationError):
            self.env['thoriumcorp.patient'].create({
                'name': 'Future Baby',
    ...
    def test_search_age(self):
        ...
        result = self.env['thoriumcorp.patient'].search(
            [('age', '=', years)]
        )
    ...
    def test_search_age_on_birthday(self, date_mock):
        ...
        result = self.env['thoriumcorp.patient'].search([('age', '=', 1)])
        self.assertIn(self.patient_1, result)
